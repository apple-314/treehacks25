%%%%%%%	   New bound combining the integrality gap and optimal solution entries


\documentclass[runningheads]{llncs}

%\textwidth15cm \textheight20cm \oddsidemargin1cm
%\evensidemargin1cm
%\frenchspacing
...
%\newtheorem{example}[thm]{Example}{\bfseries}{\normalfont}
%\newtheorem{remark}[thm]{Remark}{\bfseries}{\normalfont}
%\newtheorem{obs}[thm]{Observation}
%\newtheorem{defn}[thm]{Definition}{\bfseries}{\normalfont}
...
\newcommand{\frob}{\mbox{\rm F}}
\newcommand{\dfrob}{\mbox{\rm g}}
...
%\thanks{The second  author was funded by the Deutsche Forschungsgemeinschaft (DFG,
%German Research Foundation) under Germany's Excellence Strategy ---
%The Berlin Mathematics Research Center MATH+(EXC-2046/1, project ID: 390685689).}
\date{\today }
...
\keywords{Integrality gap  \and Proximity \and Sparsity.}
\end{abstract}
...
%The distance-sparsity transference bounds obtained in \cite{ACHW} link two active directions of research in the theory of integer programming: {\em proximity} and {\em sparsity} of solutions to integer programs. 
%%
...
%Given any vertex $\ve x^*$  of $\KP(\AA, {\ve b})$,  %
%%
%Gomory  \cite{Gomory_polyhedra}  introduced a {\em corner polyhedron} $\CP(\AA, {\ve b}, \ve x^*)$ associated with $\ve x^*$. The polyhedron $\CP(\AA, {\ve b}, \ve x^*)$ is the convex hull of the integer points in an affine polyhedral cone obtained from $\KP(\AA, {\ve b})$ by relaxing nonnegativity constraints for basic variables of the vertex $\ve x^*$. 

%Take any vertex ${\ve z}^*$ of $\CP(A, {\ve b}, \ve x^*)$. The transference bounds obtained in  \cite{ACHW} link the distance  
%between $\ve x^*$ and $\ve z^*$ measured by the $\ell_{\infty}$-norm $\|\cdot\|_{\infty}$ with the size of support of $\ve z^*$ measured by the $0$-norm $\|{\ve x}\|_0=|\{i: x_i\neq 0\}|$. In what follow, for $1\le r\le n$ we let
%\bea
%\Delta_r(\AA)=\max\{|\det \BB|: \BB \mbox{ is a }r\times r\mbox{ submatrix of }\AA\}\,.
%\eea
%We will also denote by $\gcd(\AA)$ the greatest common divisor of all $m\times m$ subdeterminants of $\AA$. Avoiding technical details, \cite[Theorem 1]{ACHW} provides a bound of the form
%\be\label{IPTP_general}
%\|{\ve x}^{*}-{\ve z}^{*}\|_{\infty} < \frac{p(m,n)\Delta_m(\AA)}{ 2^{\| {\ve z}^{*}\|_0}\gcd(\AA)}\,,
%\ee
%with $p(m,n)=2^{m+1} (n-m)$. %
%%
%On a high level, the bound \eqref{IPTP_general} implies that the distance from ${\ve x}^{*}$ to any vertex ${\ve z}^{*}$ of $\CP(A, {\ve b}, \ve x^*)$ drops exponentially in the support size $\| {\ve z}^{*}\|_0$. %

%In general, one cannot guarantee that the set of integer points in $\KP(\AA, {\ve b})$ contains any of the vertices of a corner polyhedron.  Nevertheless, for knapsack problems with positive entries %that is for $\AA\in \Z_{>0}^{1\times n}$, 
%\cite[Theorem 3]{ACHW} shows that  \eqref{IPTP_general} holds for at least one integer point ${\ve z}^{*}\in \KP(\AA, {\ve b})$. Remarkably, this gives an exponential in $\| {\ve z}^{*}\|_0$ improvement on the previously known optimal proximity bounds obtained in \cite{AHO}. 
...
%Without loss of generality, we may assume that $\AA$ has rank $m$.
%
...
linear programming relaxation %to (\ref{initial_IP}) is the problem
\begin{eqnarray}
...
%Assuming that $\KP(\AA,\bb)$ contains integer points, 
...
%We obtain transference bounds that apply to the vertices of $\KP_I(\AA,\bb)$ that correspond to optimal solutions of  (\ref{initial_IP}). 
...
%
%\begin{lemma} \label{section_volume_via_B}We have
%\bea \vol(\DD(S\cap (-1,1)^l))= |\det(\DD)|  \frac{ \Delta(\BB)}{ \Delta(\BB\DD)}\vol(S\cap (-1,1)^l)\,.\eea
%\end{lemma}
...
%
%\noindent{\bf Addition}: Just for checking, let us calculate the volume of $L$. Let ${\ve h}\in \ker(A)$ be the vector orthogonal to $\ker(B)=\ker({\ve c}^\top)\cap\ker(A)$ such that 
%${\ve c}\cdot{\ve h}={\ve c}\cdot ({\ve x}^*-{\ve z}^*)$. Since $K\subset \ker(B)$, we have
%\bea
%\vol(L)=\frac{2\vol(K)\|{\ve h}\|_2}{n-m}\,.
%\eea
%We can write ${\ve h}=t{\ve c}|_{\ker(\AA)}$, so it remains to find $t$. We have
%\bea
%{\ve c}\cdot(t{\ve c}|_{\ker(\AA)})={\ve c}\cdot ({\ve x}^*-{\ve z}^*)=IG(A, {\ve b}, {\ve c})\,.
%\eea
%Hence
%\bea
%t=\frac{IG(A, {\ve b}, {\ve c})}{{\ve c}\cdot{\ve c}|_{\ker(\AA)}}=\frac{IG(A, {\ve b}, {\ve c})}{\|{\ve c}|_{\ker(\AA)}\|^2_2}\,
%\eea
%and
%\bea
%\vol(L)=\frac{2 IG(A, {\ve b}, {\ve c})\vol(K)}{(n-m)\|{\ve c}|_{\ker(\AA)}\|_2}\,.
%\eea
...
We will derive Theorem  \ref{thm_transference_gap_integer_hull} from Corollary \ref{cor_gap_via_length}  and from the following result. %for $n>m+1$.
%
...
%For convenience, we will work with an open polyhedron 
%\bea
%\PKP(\AA, {\ve b})=\KP(\AA, {\ve b})+(-1,1)^n\cap \ker(\AA)\,.
%\eea
%Notice that $\KP(\AA, {\ve b})\cap\Z^n=\PKP(\AA, {\ve b})\cap\Z^n$.
...
\ee
%Notice that the polyhedron $\PKP(\AA, {\ve b})$ contains the set
%\bea
%K={\ve z}^*+S\,.
%\eea
%%
...
%\vol(L)\ge \frac{2^{n-m} {\ve c}\cdot({\ve z}^*-{\ve x}^*)\prod_{i=1}^{n-m-1} d_i}{(n-m)}
...
\Delta(\AA_{I\cup J})\le {\left|I\cup J\right|\choose m}^{1/2}\Delta_m(\AA_{I\cup J})\le {s+m\choose m}^{1/2}\Delta_m(\AA)\,. % <\left( \frac{n e}{m}\right)^{m/2}\Delta_m(\AA)\,.
\ee
%
...
%Given a set $K\subset\R^n$, we will also use the notation $\interior\, K$ for the interior of $K$. 
...
%
%Let us denote by $\pi$ the coordinate projection map that forgets coordinates indexed by $B$. Then 
%\be\label{projected_zonotope}
%\pi(D)=\conv(\pi(C^n({\ve 0})), \pi(C^n({\ve z}^*)))\,.
%\ee

Take any index $j\in N$. Since ${\ve w}\in D({\ve x}^*, {\ve z}^*)$, %and \eqref{projected_zonotope} holds, 
...
% % 
%Next, take any $j\in \bar \gamma$. Then, by construction of the set $D$, we have $\|x^*_j-w_j\|_2=w_j\le z_j^*$.


 
 
%  Let $A_\gamma$ be a basis of $A$ corresponding to the vertex ${\ve x}^*$. Take any index $j\in \gamma$. By construction of the set $D$, if $x^*_j\ge z^*_j$ then $x^*_j\ge w_j\ge z^*_j$ and if  $x^*_j\le z^*_j$, then $x^*_j\le w_j\le z^*_j$.
% % 
%Next, take any $j\in \bar \gamma$. Then, by construction of the set $D$, we have $\|x^*_j-w_j\|_2=w_j\le z_j^*$.
